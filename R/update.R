...
#' new_contacts_result <- dyn_create(new_contacts, "Contact")
#' 
#' update_contacts <- tibble(FirstName = rep("TestTest", n),
#'                           LastName = paste0("Contact", 1:n), 
#'                           Id = new_contacts_result$id)
#' updated_contacts_result1 <- dyn_update(update_contacts, "Contact")
#' }
#' @export
dyn_update <- function(input_data,
  ...
  input_data <- dyn_input_data_validation(operation='update', input_data)
